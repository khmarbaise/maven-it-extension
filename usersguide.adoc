...
should contain a defined content. How should that be expressed. The following gives you an example
how to do that:
[source,java]
.filename.java
----
...
----
The first check `.isSuccessful()` checks if the build was fine than we go into `project` directory
and with `withTarget()` we check the existence of the `target` directory as well as going into
that directory. And finally we append `withFile(...)` which defines which file and redirects to
