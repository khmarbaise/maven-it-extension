...
** Conditions for execution (only to execute on JDK11) and the expected result is
expressed in a separate file which in the end is a mixture of different things which violates
...
integration tests for Maven plugins etc. in a convenient way. Also is to make custom assertions with
https://assertj.github.io/doc/[AssertJ] library which makes it easier to express the intention of a
test.
