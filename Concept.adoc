...
The described above is a blocking line between the running of a Maven plugin and the running
of an integration test from within the IDE.

CAUTION: We could create a simple workaround and write a property file during the initial setup
and reuse it's content during the run of the integration tests inside the IDE.
