...
the best to express that via nested class with only a single test case
...
The beforeEach and the particular test case must be using the same case
otherwise we have no relationship between the beforeEach and the particular
test case? Is this a good idea? (We have made the assumption if not defined different
...
import org.apache.maven.jupiter.extension.MavenIT;
import org.apache.maven.jupiter.extension.MavenTest;
import org.apache.maven.jupiter.extension.maven.MavenExecutionResult;import org.junit.jupiter.api.BeforeEach;
