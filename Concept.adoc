...
:maven-release-plugin: https://github.com/apache/maven-release/tree/master/maven-release-plugin
...
In general there are some question coming up:
...
* Based on the idea parallel nature of those integration tests we need to prevent using the same
  ...
  Usually we would use `localhost:Port`.
* A repository manager can be used to deploy artifacts (during a test) into it and afterwards check
  the content somehow. (For example if checksum have been correctly created).
