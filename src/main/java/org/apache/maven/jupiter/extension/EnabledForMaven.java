...
/**
 * {@code @EnabledForMaven} is used to signal that the annotated test class or test method is <em>enabled</em> on one
 * or more specified Maven version which the tests are being run with.
 *
 * <p>When applied at the class level, all test methods within that class
 * will be enabled on the same specified Maven version.
 *
...
@Documented
@ExtendWith(EnabledForMavenCondition.class)
@API(status = EXPERIMENTAL, since = "0.1")
public @interface EnabledForMaven {
