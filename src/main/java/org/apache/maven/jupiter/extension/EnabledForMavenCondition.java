...
/**
 * {@link ExecutionCondition} for {@link EnabledForMaven @EnabledForMaven}.
 *
 * @author Karl Heinz Marbaise
 * @see EnabledForMaven
 * @since 0.1
 */
class EnabledForMavenCondition implements ExecutionCondition {
  ...
  public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
    Optional<EnabledForMaven> optional = findAnnotation(context.getElement(), EnabledForMaven.class);
    if (optional.isPresent()) {
      MavenVersion[] versions = optional.get().value();
      Preconditions.condition(versions.length > 0, "You must declare at least one version in @EnabledForMaven");
