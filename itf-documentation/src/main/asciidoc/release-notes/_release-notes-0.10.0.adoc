...
:issue-187: https://github.com/khmarbaise/maven-it-extension/issues/187[Fixed #187]
...
*Scope:*
 ...
 - Refactoring/Code cleanups
   ...
   * {issue-187} - Replace MavenProjectResult with more appropriate Information.
 ...
 * {issue-187} - The whole class `MavenProjectResult` is marked deprecated and will be
   removed with release 0.11.0. This means you have to replace the usage of
   `MavenProjectResult` with one of the appropriate replacements like `MavenTarget`,
   `MavenSource`, `MavenModelSource` or `MavenModelTarget` (details can be looked
    up in the users guide).
