...
Sometimes it happens that you need to do the testing of your plugin
 in a different way than usually. For example you would like to
make the setup programmatically or in other ways.

The default setup assumes that your projects which are used for the
integration testings are put into the `resources-its` location
and copied from that location into the `target/maven-its`
...
==== Use Case
If you like to generate your project setup programmatically you can turn of the
requirement to copy the project from `resources-its/.../` via
`@MavenProjectSources(resourcesUsage=NONE)` that means you have to provide a
project setup on your own. The location where you need to create the setup can be
calculated by using the parameters which are injected in the `beforeEach`
method. The location where you need to create your project content,
in particular `pom.xml` is given by `result.getTargetProjectDirectory()` (The injection
into `beforeEach` method) within that directory you have to do the whole setup.
...
The usage should be selected to use a single directory to
be used for all executed project instead of using
on per method / class / nested class or alike.

This means we will search in the `src/test/resources-its/`
plus given the source location to find the source of the project
which will be used for running the integration test.

Defining on the class level means that the given location
will be used for all tests within the class or it's nested
classes.

Intention: Reuse the same project setup for different test
executions.
...
Defining different source location for different
tests for different testmethods?

The following examle uses the same location for the
tests `one` and `two`.
...
The following example will define to use a common location for the
project sources which is used by `one` and `two` while
`three` and `four` will use a common location `com/soebes/base-nested`.
