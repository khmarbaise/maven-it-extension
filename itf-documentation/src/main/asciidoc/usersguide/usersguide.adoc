...
Consume the project based on package + ClassName + MethodName to find the
appropriate project sources which are used to execute the tests.
...
Do not use any project resources at all. Define it on your own
project location. A good setup will be using the `@BeforeEach` method to
setup the test.
...
@MavenJupiterExtension
@MavenProjectSources(resourcesIts=NO)
class TheIT {
