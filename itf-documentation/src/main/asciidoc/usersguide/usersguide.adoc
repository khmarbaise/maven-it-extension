...
`mvn-stdout.log`(stdout) and the output to stderr is written to `mvn-stderr.log`. The used
 line parameters command to call Maven are wrote into `mvn-arguments.log`.
...
also support replacement of placeholders where currently the following are supported:
...
[source,java]
----
@MavenTest( options = {MavenCLIOptions.NON_RECURSIVE, "--offline"},
...
class SetIT
{
    ...
    class set_001
    {
        ...
        @MavenTest(
            options = MavenCLIOptions.NON_RECURSIVE,
